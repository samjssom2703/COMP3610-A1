...
def load_data():
    if not os.path.exists(CLEAN_PATH):
        st.error("Cleaned dataset not ready yet. Please visit the Home page first to trigger data processing.")
        st.stop()
...
def load_zones():
    if not os.path.exists(ZONE_CSV):
        st.error("Zone lookup CSV not found. Please visit the Home page first.")
        st.stop()
